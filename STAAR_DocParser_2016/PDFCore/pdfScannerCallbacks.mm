...
void didScanNewLine(CGPDFScannerRef pdfScanner, Scanner2 *scanner, BOOL persistLeading) {
	CGPDFReal tx, ty;
...
void newLineWithLeading(CGPDFScannerRef pdfScanner, void *info) {
	didScanNewLine(pdfScanner, (__bridge Scanner2 *) info, NO);
}

void newLineSetLeading(CGPDFScannerRef pdfScanner, void *info) {
	didScanNewLine(pdfScanner, (__bridge Scanner2 *) info, YES);
}
